...
start_ollama() {
    if curl -fsS --connect-timeout 1 --max-time 2 "$OLLAMA_URL/api/tags" >/dev/null 2>&1; then
        echo "✓ Ollama already available at $OLLAMA_URL"; return 0
    fi
    if ! command -v ollama >/dev/null 2>&1; then echo "✗ Ollama executable not found." >&2; return 1; fi
    ...
    if ! write_identity "$!" "$OLLAMA_ID_FILE"; then
        ...
        rm -f "$OLLAMA_PID_FILE" "$OLLAMA_ID_FILE"; return 1
    fi
    if ! wait_for_http "$OLLAMA_URL/api/tags" 30; then echo "✗ Ollama failed to become ready." >&2; return 1; fi
    echo "✓ Ollama ready"
}

start_memory() {
    if curl -fsS --connect-timeout 1 --max-time 2 "$CHAT_URL" >/dev/null 2>&1; then
        echo "✓ Memory injector already available at $CHAT_URL"; return 0
    fi
    ...
    if ! write_identity "$!" "$MEMORY_ID_FILE"; then
        ...
        rm -f "$MEMORY_PID_FILE" "$MEMORY_ID_FILE"; return 1
    fi
    if ! wait_for_http "http://127.0.0.1:5000/health" 30; then echo "✗ Memory injector failed to become ready." >&2; return 1; fi
...
stop_one() {
    ...
    if ! [ -f "$pid_file" ] || ! [ -f "$identity_file" ]; then
        echo "✗ Cannot stop $name: controller ownership state is incomplete." >&2; return 1
    fi
    if ! process_identity_matches "$pid_file" "$identity_file"; then
        echo "✗ Cannot stop $name: process identity does not match controller ownership." >&2; return 1
    fi
    local pid; pid=$(cat "$pid_file")
    echo "Stopping $name (PID $pid)..."; kill "$pid" 2>/dev/null || true
    for _ in $(seq 1 10); do kill -0 "$pid" 2>/dev/null || break; sleep 1; done
    if kill -0 "$pid" 2>/dev/null; then echo "✗ $name did not terminate cleanly." >&2; return 1; fi
...
stop_runtime() {
    ...
    if [ -f "$MEMORY_PID_FILE" ] || [ -f "$MEMORY_ID_FILE" ]; then
        ...
        echo "↷ Memory injector left running: externally managed." >&2; failed=1
    else
        echo "↷ Memory injector not running under controller ownership." >&2; failed=1
    fi
    ...
    if [ -f "$OLLAMA_PID_FILE" ] || [ -f "$OLLAMA_ID_FILE" ]; then
        ...
        echo "↷ Ollama left running: externally managed." >&2; failed=1
    else
        echo "↷ Ollama not running under controller ownership." >&2; failed=1
    fi

    if [ "$failed" -ne 0 ]; then echo "PROJECT-NAS runtime: STOPPED WITH ERRORS" >&2; return 1; fi
    echo "PROJECT-NAS runtime: STOPPED"
}

status_runtime() {
    echo "=== PROJECT-NAS RUNTIME ==="
    if curl -fsS --connect-timeout 1 --max-time 2 "$OLLAMA_URL/api/tags" >/dev/null 2>&1; then echo "✓ Ollama      $OLLAMA_URL"; else echo "✗ Ollama      unavailable"; fi
    if curl -fsS --connect-timeout 1 --max-time 2 "http://127.0.0.1:5000/health" >/dev/null 2>&1; then echo "✓ Memory API  http://127.0.0.1:5000"; else echo "✗ Memory API  unavailable"; fi
    if process_identity_matches "$MEMORY_PID_FILE" "$MEMORY_ID_FILE"; then echo "✓ Controller  memory PID $(cat "$MEMORY_PID_FILE")"; fi
    if process_identity_matches "$OLLAMA_PID_FILE" "$OLLAMA_ID_FILE"; then echo "✓ Controller  Ollama PID $(cat "$OLLAMA_PID_FILE")"; fi
}

doctor_runtime() { python "$PROJECT_ROOT/runtime/doctor.py"; }

chat_runtime() {
    for cmd in curl jq; do if ! command -v "$cmd" >/dev/null 2>&1; then echo "Error: '$cmd' is required but not installed." >&2; exit 1; fi; done
    if [ ! -f "$MASTER_PROMPT_FILE" ]; then echo "Error: canonical prompt not found: $MASTER_PROMPT_FILE" >&2; exit 1; fi
    CONTEXT=$(cat "$MASTER_PROMPT_FILE")
    read -r -p "Enter your command or question: " USER_INPUT
    if [ -z "$USER_INPUT" ]; then echo "Error: no input given." >&2; exit 1; fi
    ...
    if ! RESPONSE=$(curl --fail-with-body -sS --connect-timeout 5 --max-time 120 -X POST "$CHAT_URL" -H "Content-Type: application/json" -d "$PAYLOAD"); then
        echo "Error: could not complete request to $CHAT_URL. Run 'runtime/project-nas.sh start'." >&2; exit 1
    fi
    echo "$RESPONSE" | jq -r '.response // .error // "No response field returned."'
}

case "${1:-}" in
    start) start_runtime ;; stop) stop_runtime ;; restart) stop_runtime; start_runtime ;; status) status_runtime ;; doctor) doctor_runtime ;; chat) chat_runtime ;; -h|--help|help) usage ;; *) usage; exit 2 ;;
esac
