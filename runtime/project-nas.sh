...
start_ollama() { if curl -fsS --connect-timeout 1 --max-time 2 "$OLLAMA_URL/api/tags" >/dev/null 2>&1; then echo "✓ Ollama already available at $OLLAMA_URL"; return 0; fi; if ! command -v ollama >/dev/null 2>&1; then echo "✗ Ollama executable not found." >&2; return 1; fi; echo "Starting Ollama..."; nohup ollama serve >>"$OLLAMA_LOG" 2>&1 & local pid=$!; write_pid "$pid" "$OLLAMA_PID_FILE"; if ! write_identity "$pid" "$OLLAMA_ID_FILE"; then echo "✗ Could not establish Ollama process identity." >&2; rm -f "$OLLAMA_PID_FILE" "$OLLAMA_ID_FILE"; return 1; fi; if ! wait_for_http "$OLLAMA_URL/api/tags" 30; then echo "✗ Ollama failed to become ready." >&2; return 1; fi; echo "✓ Ollama ready"; }
start_memory() { if curl -fsS --connect-timeout 1 --max-time 2 "$MEMORY_HEALTH_URL" >/dev/null 2>&1; then echo "✓ Memory injector already available at $MEMORY_HEALTH_URL"; return 0; fi; echo "Starting PROJECT-NAS memory injector..."; nohup python "$PROJECT_ROOT/runtime/memory_injector.py" >>"$MEMORY_LOG" 2>&1 & local pid=$!; write_pid "$pid" "$MEMORY_PID_FILE"; if ! write_identity "$pid" "$MEMORY_ID_FILE"; then echo "✗ Could not establish memory injector process identity." >&2; rm -f "$MEMORY_PID_FILE" "$MEMORY_ID_FILE"; return 1; fi; if ! wait_for_http "$MEMORY_HEALTH_URL" 30; then echo "✗ Memory injector failed to become ready." >&2; return 1; fi; echo "✓ Memory injector ready"; }
