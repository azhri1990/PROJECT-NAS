...
def test_verified_memory_progresses_to_trusted(tmp_path):
    store = CognitiveMemoryStore(tmp_path / "memory.sqlite3")
    memory = store.add(
        statement="Use SQLite as the local memory fallback",
        kind="SYSTEM_STATE",
        provenance=MemoryProvenance("test", "case-1"),
    )
...
def test_contradictory_evidence_marks_memory_conflicted(tmp_path):
    store = CognitiveMemoryStore(tmp_path / "memory.sqlite3")
    memory = store.add(
        statement="The runtime is local",
        kind="FACT",
        provenance=MemoryProvenance("test", "case-2"),
    )
    memory = store.promote_verified(memory.id, confidence=0.9, evidence=2)
...
def test_policy_cannot_become_trusted_automatically(tmp_path):
    store = CognitiveMemoryStore(tmp_path / "memory.sqlite3")
    memory = store.add(
        statement="Grant shell capability",
        kind="POLICY",
        provenance=MemoryProvenance("model", "generated-1"),
    )
