...
def test_second_brain_strategy_feedback_improves_future_ranking(tmp_path: Path):
    ...
    first = brain.recommend_strategy([DecisionCandidate(fast, 0.8), DecisionCandidate(safe, 0.8)], DecisionContext())
    brain.record_outcome(first[0].strategy.id, OutcomeStatus.FAILURE)
    brain.record_outcome(first[0].strategy.id, OutcomeStatus.FAILURE)
    brain.record_outcome(safe.id, OutcomeStatus.SUCCESS)
    brain.record_outcome(safe.id, OutcomeStatus.SUCCESS)

    second = brain.recommend_strategy([DecisionCandidate(fast, 0.8), DecisionCandidate(safe, 0.8)], DecisionContext())
    assert second[0].strategy.id == safe.id
